...
from ..models.assets import Asset, QuoteAsset, AssetSchema, QuoteAssetSchema
...
class RetrieverPipeline(RetriverMixin):
    ...
    def __init__(self, symbols: List[str], lower_range: str = None, upper_range: str = None):
        self.lower_range = None if not lower_range else marshal_date(
            lower_range)
        self.upper_range = None if not upper_range else marshal_date(
            upper_range)
